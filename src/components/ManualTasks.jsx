...
const ManualTasks = () => {
        ...
        if (newCountdown <= 0) {
            ...
            <div className="w-full flex justify-center flex-col items-center space-y-3">
              <div className="w-full items-center justify-center flex flex-col space-y-2">
                ...
                <p className="font-medium text-center">
                  Wait 30 minutes for moderation check to claim bonus!!
                </p>
              </div>
              <p className="pb-6 text-[#9a96a6] text-[15px] w-full text-center">
                If you have not performed this task make sure you do so within
                30 minutes to claim your bonus!
              </p>
            </div>
          );
          setModalOpen(true);
          ...
          const saveTaskToUser = async () => {
            try {
              ...
              console.log(
                `Task ${taskId} added to user's manualTasks collection`
              );
            } catch (error) {
  ...
  useEffect(() => {
    const submittedStates = manualTasks.reduce((acc, task) => {
      const submittedState =
        localStorage.getItem(`submitted_${task.id}`) === "true";
  ...
  const renderManualTasks = () => {
    if (!manualTasks?.length) {
      return null; // Don't render anything if there are no manual tasks
    }
            ...
            <div className="flex h-full w-full flex-col bg-[#2d2d2d] justify-center rounded-[24px] py-4 pl-12 pr-4 relative">
              ...
              <div className="flex w-full flex-col justify-between h-full space-y-2">
                ...
                <span className="flex items-center w-fit space-x-1 text-primary text-[14px] font-semibold">
                  <span
                    className={`w-[10px] h-[10px] bg-accent rounded-full flex items-center`}
                  ></span>
                      ...
                      {countdowns[task.id] === undefined && (
                        <button
                          ...
                          className={`${
                            submitted[task.id]
                              ? `bg-btn4 text-[#000]`
                              : buttonText[task.id] || `bg-btn4 text-[#000]`
                          } ${
                            !showVerifyButtons[task.id]
                              ? "!bg-btn2 !text-[#888]"
                              : `bg-btn4 text-[#000]`
                          } w-fit py-[6px] px-4 font-medium rounded-[6px]`}
                          disabled={!showVerifyButtons[task.id]}
                        >
                          {submitted[task.id]
                            ? "Verifying"
                            : buttonText[task.id] || "Verify"}
                        </button>
                      )}
                    </>
                  )}
